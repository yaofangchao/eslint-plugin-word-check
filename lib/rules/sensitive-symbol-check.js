...
let sensitiveSymbols = []
  ...
  const reportMsgByNode = function(node, context){
    if((node.type === 'VText' || node.type === 'Literal') && typeof(node.value) === 'string'){
      if(!sensitiveSymbols || !sensitiveSymbols.length || !node || !node.value || !node.value.length) {
        return
      }
  ...
  module.exports = {
    ...
    create: function(context) {
      sensitiveSymbols = (context.options && context.options.length && context.options[0].sensitiveSymbols) ? context.options[0].sensitiveSymbols : []
